...
### RAG的本質：將資料整理好放入你的專用圖書館，然後在你詢問時通過優化的檢索手續提取正確的知識
...
**❌ 知識局限性問題** → **✅ 專業對口的資料和具有時效性的資料**

一般LLM是通用的大語言模型，知識廣泛但專業性不強，而且資料的時效性截止於訓練模型的日期。RAG的檢索生成是基於我們自己放入資料庫的資料，可以提高專業性、也可以隨時更新。

**❌ 容易產生「幻覺」** → **✅ 可追溯性與可靠性**

一般LLM找不到答案時不會誠實說「我不知道」，而是基於相似資訊推斷，生成看似合理但實際錯誤的答案。RAG搭配向量資料庫可以完全溯源，將來源資料編號和文件提供出來。

**❌ 缺乏領域專業知識** → **✅ 企業知識整合與成本效益**

LLM訓練於公開資料，無法獲得企業內部專有知識、標準作業程序、客戶資料等私有資訊。RAG可整合任何企業內部資料；相比重新訓練模型，維護RAG系統成本更低且更新速度更快。
